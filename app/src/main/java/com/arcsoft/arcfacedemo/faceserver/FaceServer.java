...
public class FaceServer {
     ...
     *         NV21高度
     * @param faceInfo
     *         {@link FaceEngine#detectFaces(byte[], int, int, int, List)}获取的人脸信息
     * @param name
     *         保存的名字，若为空则使用时间戳
     *
     * @return 是否注册成功
     */
    public boolean registerNv12(Context context, byte[] nv21, int width, int height, FaceInfo faceInfo, String name) {
        synchronized (this) {
            ...
            //            String id = idCardNo == null ? String.valueOf(System.currentTimeMillis()) : idCardNo;
...
//            }
            ...
            int code = faceEngine.detectFaces(bgr24, width, height, FaceEngine.CP_PAF_NV21, faceInfoList);
    ...
    public CompareResult getTopOfFaceLib(FaceFeature faceFeature) {
        ...
        if (maxSimilarIndex != -1) {
            FaceRegisterInfo faceRegisterInfo = faceRegisterInfoList.get(maxSimilarIndex);
            return new CompareResult(faceRegisterInfo.getName(), maxSimilar);
        }
        return null;
    }
}
